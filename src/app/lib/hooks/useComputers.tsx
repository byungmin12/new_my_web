...
import { TMesh } from '@/app/lib/types/type';
...
interface IScreenGeo {
  position?: Vector3;
  rotation?: Euler;
  scale?: Vector3;
}

interface IAppliance extends IScreenGeo {
  geometry: BufferGeometry<NormalBufferAttributes>;
  material: Material | Material[];
}

interface IMonitor extends IScreenGeo {
  type: 'blank' | 'route';
  frame: TMesh;
  panel: TMesh;
...
function useComputers(frameId?: string, panelId?: string) {
  ...
  const appliances: IAppliance[] = useMemo(
    () => [
      {
  ...
  const monitors: IMonitor[] = useMemo(
    () => [
      {
        type: 'blank',
        frame: nodes['Object_212'],
        panel: nodes['Object_213'],
        position: [-2.731, 0.629, -0.522],
        rotation: [0, 1.087, 0],
      },
      {
        type: 'blank',
        frame: nodes['Object_209'],
        panel: nodes['Object_210'],
        position: [-1.43, 2.496, -1.8],
        rotation: [0, 1.002, 0],
      },
      {
        type: 'blank',
        frame: nodes['Object_221'],
        panel: nodes['Object_222'],
      ...
      {
        type: 'blank',
        frame: nodes['Object_218'],
        panel: nodes['Object_219'],
      ...
      {
        type: 'blank',
        frame: nodes['Object_215'],
        panel: nodes['Object_216'],
        position: [1.845, 0.377, -1.771],
        rotation: [0, -Math.PI / 9, 0],
      },
      {
        type: 'blank',
        frame: nodes['Object_224'],
        panel: nodes['Object_225'],
        position: [-3.899, 4.287, -2.642],
        rotation: [0, 0.539, 0],
      },
      {
        type: 'blank',
        frame: nodes['Object_227'],
        panel: nodes['Object_228'],
        position: [0.955, 4.282, -4.203],
        rotation: [0.003, -0.647, 0.003],
      },
      {
        type: 'blank',
        frame: nodes['Object_230'],
        panel: nodes['Object_231'],
...

      {
        ...
        position: [4.683, 4.29, -1.558],
        rotation: [0, -Math.PI / 3, 0],
        children: <Image url='/ic_id_card.png' position={[-3.15, 0.75, 0]} transparent={true} scale={1.5} />,
        annotation: <div>test</div>,
      },
      {
        type: 'route',
        frame: nodes['Object_206'],
        panel: nodes['Object_207'],
